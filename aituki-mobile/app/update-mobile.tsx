/**
 * Update Mobile Number Screen
 * Matches Figma design exactly - form with two mobile number inputs for confirmation
 */
...
export default function UpdateMobileScreen() {
  const router = useRouter();
  const { profile, loading: profileLoading, updateMobile } = useUserProfile();
  ...
  useEffect(() => {
    if (profile?.mobile_number) {
      ...
      setConfirmMobileNumber(profile.mobile_number);
    }
  }, [profile]);

  const handleSubmit = async () => {
    if (!mobileNumber.trim() || !confirmMobileNumber.trim()) {
      Alert.alert('Error', 'Please fill in both mobile number fields');
      return;
    }
    ...
    setSaving(true);
    try {
      const success = await updateMobile(mobileNumber.trim());
      setSaving(false);
      ...
      if (success) {
        ...
        Alert.alert('Error', 'Failed to update mobile number. Please try again.');
      }
    } catch (error) {
      setSaving(false);
      Alert.alert('Error', 'An error occurred while updating your mobile number.');
        ...
        >
          ...
          >
            ...
            <View style={styles.formSection}>
              {/* First Mobile Number Input */}
              <View style={styles.inputWrapper}>
                <View style={styles.labelContainer}>
                  <Text style={styles.label}>Enter new mobile number</Text>
                </View>
                <TextInput
                  style={styles.input}
              ...
              {/* Second Mobile Number Input - Confirmation */}
              <View style={styles.inputWrapper}>
                <View style={styles.labelContainer}>
                  <Text style={styles.label}>Reenter new moboie number</Text>
                </View>
                <TextInput
                  style={styles.input}
                  placeholder="e.g. +44 000 0000"
